...
static void touch_read(esp_lcd_touch_handle_t touch_handle, lv_indev_data_t * data)
{
  uint16_t x = 0;
  uint16_t y = 0;
  uint16_t strength = 0;
  ...
  bool touched = esp_lcd_touch_get_coordinates(touch_handle, &x, &y, &strength, &touch_count, 1);
...
static void touch1_read_cb(lv_indev_drv_t * drv, lv_indev_data_t * data)
{
  (void)drv;
  touch_read(tp, data);
}

static void touch2_read_cb(lv_indev_drv_t * drv, lv_indev_data_t * data)
{
  (void)drv;
  touch_read(tp2, data);
}
